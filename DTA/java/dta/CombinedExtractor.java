...
public final class CombinedExtractor implements FeatureExtractor{

       public static int FEAT_NUM;
       public static final int LAB_NUM = 1;

       public static int AFTER_STOP_MAX_SEC = 5;
       ...
       public CombinedExtractor(){
             FEAT_NUM = Route.SPEED_NTILES_NUM
                     ...
                     + REAfterStop2.SECS_TO_MEASURE.length
                     + Route.LAGSPD_NTILES_NUM /* lag2 speed */
                     + Route.LAGSPD_NTILES_NUM /* lag3 speed */
						+ Route.GPDST_NTILES_NUM /* gplot dist ntiles */
       ...
       public double[] extractFeatures(Route rt){
             ...
             s = rt.getLag2SpdNTiles();
             for(int i=0;i<Route.LAGSPD_NTILES_NUM;i++){
                    ret[pos] = s[i];
                    pos++;
             }
             s = rt.getLag3SpdNTiles();
             for(int i=0;i<Route.LAGSPD_NTILES_NUM;i++){
                    ret[pos] = s[i];
                    pos++;
             }
             s = rt.getGpDstNTiles();
             for(int i=0;i<Route.GPDST_NTILES_NUM;i++){
                    ret[pos] = s[i];
                    pos++;
             }
